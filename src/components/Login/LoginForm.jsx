...
const LoginForm = () => {
  ...
  const postLogin = async () => {
    try {
      ...
      try {
        const response = await authInstance.get("/competition");
    ...
    <div className="flex flex-col w-full items-center justify-center pt-14 pb-20 gap-3">
      ...
      <form className="flex flex-col items-center" onSubmit={handleSubmit}>
        <div className="flex flex-col pb-3">
          ...
          <input
            className="w-72 h-12 border-2 border-gray-200 rounded-md mt-1 p-3 text-sm"
            type="email"
        ...
        <div>
          <button className="w-72 h-11 bg-yellow-200 rounded-md hover:bg-yellow-300 font-bold">로그인</button>
        </div>
      </form>

      {/* 비밀번호찾기 회원가입 */}
      <div className="w-72 flex flex-row justify-evenly items-center text-center text-base text-gray-400">
        <div className="flex-1 text-center font-bold">비밀번호 찾기</div>
        <span> | </span>
        <div role="button" className="flex-1 text-center font-bold">
          <Link to={"/signup"}>
            <div>회원가입</div>
          </Link>
        </div>
