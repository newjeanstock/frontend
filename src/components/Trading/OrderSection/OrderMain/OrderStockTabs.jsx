...
const OrderStockTabs = ({ getActiveTabOption }) => {
  ...
  return (
    <>
      <div className="mb-4 border-b border-gray-200 font-bold">
        <ul
        ...
        >
          {tabsData.map((tab) => (
            <li className="flex-auto" role="presentation" key={tab.id}>
              <button
                className={`inline-block w-full p-3 border-b-2 rounded-t-lg font-bold text-sm ${
                  activeTab === tab.id
                    ? activeTab === "buy"
                      ? "font-bold text-red-500 border-red-500"
                      : "font-bold text-blue-600  border-blue-600"
                    : "font-bold text-gray-500 border-transparent hover:text-gray-600 hover:border-gray-300 "
                }`}
                id={`${tab.id}-tab`}
                type="button"
                role="tab"
