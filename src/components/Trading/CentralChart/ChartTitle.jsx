  ...
  return (
    <div className="flex justify-between items-center mx-8 mt-3 mb-6">
      <div className="flex gap-3 items-center">
        ...
        <div className="text-lg font-bold text-gray-500">{stockId}</div>
      </div>
      ...
      <div className={`flex gap-4 items-center ${txtColor}`}>
        <span className="text-2xl font-bold">
          {isNaN(parseInt(stockInfo.currentPrice)) ? "-" : parseInt(stockInfo.currentPrice).toLocaleString()}
        </span>
        ...
        <span className="text-lg font-bold">
          {changeSign}
          {isNaN(Math.abs(stockInfo.change)) ? "" : Math.abs(stockInfo.change)}
        </span>
      </div>
    </div>
  );
