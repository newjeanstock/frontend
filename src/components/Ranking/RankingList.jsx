...
function EachRanking({ user }) {
  ...
  const cumProfitRate = ((user.profit / (user.competitionCount * 50000000)) * 100 || 0).toFixed(1);

  return (
    <div className="grid grid-cols-4 text-center py-3 items-center font-bold text-gray-500">
...
const RankingList = ({ memberList }) => {
  ...
  return (
    <div className="mx-12">
      <div className="grid grid-cols-4 text-center font-bold py-4 mt-2">
      ...
      {memberList.map((user, index) => (
        <div className="hover:bg-slate-100 hover:text-gray-800" key={index}>
